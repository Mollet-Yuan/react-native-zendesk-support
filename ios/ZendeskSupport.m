...
RCT_EXPORT_MODULE(ZendeskSupport);
...
- (ZDKChatAPIConfiguration*)applyVisitorInfo:(NSDictionary*)options intoConfig:(ZDKChatAPIConfiguration*)config {
	...
	NSLog(@"[ZendeskSupport] Applied visitor info: department: %@ tags: %@, email: %@, name: %@, phone: %@", config.department, config.tags, config.visitorInfo.email, config.visitorInfo.name, config.visitorInfo.phoneNumber);
	return config;
}

#define RNZDKConfigHashErrorLog(options, what)\
if (!!options) {\
	NSLog(@"[ZendeskSupport] Invalid %@ -- expected a config hash", what);\
}
...
RCT_EXPORT_METHOD(startChat:(NSDictionary *)options) {
	if (!options || ![options isKindOfClass: NSDictionary.class]) {
		if (!!options){
			NSLog(@"[ZendeskSupport] Invalid JS startChat Configuration Options -- expected a config hash");
		}
	...
	dispatch_sync(dispatch_get_main_queue(), ^{
		...
		if (!!error) {
			NSLog(@"[ZendeskSupport] Internal Error loading ZDKChatEngine %@", error);
			return;
		}
		...
		UIViewController *viewController = [ZDKMessaging.instance buildUIWithEngines:engines
																 configs:@[chatConfig, messagingConfig]
																   error:&error];
		if (!!error) {
			NSLog(@"[ZendeskSupport] Internal Error building ZDKMessagingUI %@",error);
			return;
		}
